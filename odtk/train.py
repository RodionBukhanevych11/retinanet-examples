...
def train(model, state, path, annotations, val_path, val_annotations, resize, max_size, jitter, batch_size, iterations,
    ...
    for iteration in range(iterations):
        ...
        for data, target, _, _ in tqdm(data_iterator):
            # Forward pass
            optimizer.zero_grad()
            start = time.time()
            if with_apex:
            ...
            if not isfinite(cls_loss + box_loss):
        ...
        if val_annotations:
            stats = infer(model, val_path, None, resize, max_size, batch_size, annotations=val_annotations,
                            mixed_precision=mixed_precision, world=world, use_dali=use_dali,
                            with_apex=with_apex, is_validation=True, verbose=False, rotated_bbox=rotated_bbox, nc = nc)
